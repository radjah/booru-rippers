...
for i in `cat get.pixiv.pics.alt.txt get.pixiv.pics.new.txt`
do
  wget "http://www.pixiv.net/member_illust.php?mode=big&illust_id=$i" --load-cookies=pixiv.txt --referer="http://www.pixiv.net/member_illust.php?mode=medium&illust_id=$i" -O -|pcregrep -o  -e 'http\:\/\/i\d{1,3}\.pixiv\.net\/img[^\"]+' >> get.pixiv.pics.dl.txt
done;
...
comm -2 -3 get.pixiv.album.dld.txt get.pixiv.album.dl.alt.txt|sort|uniq -u > get.pixiv.album.small.txt
